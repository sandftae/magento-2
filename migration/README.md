...
#### migrate:split
## SPLIT таблиц
именно об этой части говорится по "прологе".
Для реализвции было написано свое расширение `data-migration-tool`. Это расширение включает:
...
`Расширение не может считаться закоченным, т.к. не имеет написаных тест-кейсов и не было должным образом протестировано (тестирование проводилось только на версии 1.9.3.10 М1). Тестировалось только локально. Расширение 'сырое и не для продакшина', а также работает сейчас только с версией magento 2.3` 
